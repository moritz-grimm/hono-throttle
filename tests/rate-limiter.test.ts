...
describe("rateLimiter", () => {
    ...
    test("returns 429 if keyGenerator maps every request to the same key", async() => {
        const app = createApp({ keyGenerator: () => "shared" });
    ...
    test("returns 200 if keyGenerator ignores x-forwarded-for", async() => {
        const app = createApp({ keyGenerator: (c) => `token:${c.req.header("x-api-key")}` });

        for (let i = 0; i < 10; i++) {
            await app.request("/", {
                headers: { ...headers, "x-api-key": "first" },
            });
        }

        const blocked = await app.request("/", {
            headers: { "x-forwarded-for": "9.9.9.9", "x-api-key": "first" },
        });
        ...
        const allowed = await app.request("/", {
            headers: { ...headers, "x-api-key": "second" },
        });

        expect(allowed.status).toBe(200);
    });

    test("returns 200 on a whitelisted key generated by keyGenerator", async() => {
        const app = createApp({
            keyGenerator: (c) => `token:${c.req.header("x-api-key")}`,
            whitelist: [ "token:internal" ],
        });

        for (let i = 0; i < 10; i++) {
            await app.request("/", {
                headers: { ...headers, "x-api-key": "internal" },
            });
        }
    ...
    test("returns 200 if skipPaths matches, without calling keyGenerator", async() => {
        const keyGenerator = vi.fn(() => "shared");
        const app = createApp({ keyGenerator: keyGenerator, skipPaths: [ "/status" ] });
